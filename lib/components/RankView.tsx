  ...
  return (
    <div className={`fullHeight fullWidth topLeft absolute flexCenter flexDirColumn rankView`}>
      ...
      <ul className = "rankList">
        {rankData.length > 0 ? 
        rankData.map((item: rankItem, index)=><li className = "noClick"><p className = "click userText" onClick = {()=>{navigateToUser(item.username)}}>{`${index + 1}. ${item.username.toUpperCase()}`}</p><p>{`SCORE: ${item.score} second(s)`}</p></li>)

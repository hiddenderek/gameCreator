    ...
    return (
        <div className="ranks content" >
            <div className = "rankSection">
                ...
                <ul>{typeof highScoreRanks.map === "function" ? highScoreRanks.map((item: userData, index)=><li className = "rankEntry"><p>{`${index + 1}: ${item.username}`}</p><p className = "right">{`${item.score_count} high scores`}</p></li>) : ""}</ul>  
            </div>
            <div className = "rankSection">
                <h1 className = "fullWidth flexCenter smallHeight rankLabel">LIKES RECIEVED</h1>
                <ul>{typeof likeRanks.map === "function" ? likeRanks.map((item: userData, index)=><li className = "rankEntry"><p>{`${index + 1}: ${item.username}`}</p><p className = "right">{`${item.total_score} likes`}</p></li>) : ""}</ul>  
            </div>
            <div className = "rankSection">
                <h1 className = "fullWidth flexCenter smallHeight rankLabel">PLAYS RECIEVED </h1>
                <ul>{typeof playRanks.map === "function" ? playRanks.map((item: userData, index)=><li className = "rankEntry"><p>{`${index + 1}: ${item.username}`}</p><p className = "right">{`${item.play_count} plays`}</p></li>) : ""}</ul>  
            </div>
        </div>
    );
}

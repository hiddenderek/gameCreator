        ...
        renderWithRouter(
            <Router>
                <Provider store={store}>
                    <GameIcon index={0} gameData={{ game_name: testGameData[0].game_name, plays: testGameData[0].plays, src: "", user_id: testGameData[0].userId }} profileData={{ username: "testUser0" }} getData={() => { }} />
                </Provider>
            </Router>
            , { route: "/games" })
    })
...
test('Game Icon Populate', async () => {
    const user = userEvent.setup()
    const gameIconTitleElm = screen.getByTestId('game_icon_name_0')
    expect(gameIconTitleElm.textContent).toMatch(testGameData[0].game_name.toUpperCase())
    const gameIconPlaysElm = screen.getByTestId('game_icon_plays_0')
    expect(gameIconPlaysElm.textContent).toMatch(testGameData[0].plays.toString())
    const gameIconUserElm = screen.getByTestId('game_icon_user_0')
    expect(gameIconUserElm.textContent).toMatch(testGameData[0].username)
})
